...
Pre simulačný model je využitá diskrétna simulácia\cite[str. 34]{IMS}. Tento model je vytvorený v programovacom jazyku Python3. Dôvodom pre využitie programovacieho jazyka Python3 je možnosť využitia vedeckých knižníc Numpy a Scipy. Tieto knižnice obsahujú matematické metódy, ktoré zjednodušujú spracovanie vstupných dát a poskytujú možnosť aproximovať krivky. Ak alternatívu by bolo možné použiť knižnicu GNU Scientific Library jazyka C, ktorá tiež poskytuje vedecké matematické metódy. Z dôvodu vizualizácie a jednoduchého načítania dát sme zvolili Python.%zdôvodnit preco nie, vizualizacia?
